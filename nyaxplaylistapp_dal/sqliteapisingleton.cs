    ...
    public sealed class sqliteapisingleton
    {
        ...
        public responsedto createtables(sqliteconnectionstringdto _connectionstringdto)
        {
            ...
            try
            {
                ...
                bool does_table_exist_in_db = checkiftableexists(CONNECTION_STRING, DBContract.playlist_entity_table.TABLE_NAME);

                //Create the table 
                string SQL_CREATE_CROPS_TABLE = " CREATE TABLE IF NOT EXISTS " + DBContract.playlist_entity_table.TABLE_NAME + " (" +
                      DBContract.playlist_entity_table.MEDIA_ID + " INTEGER PRIMARY KEY, " +
        ...
        public List<playlist_dto> getallweights()
        {
            List<playlist_dto> lst_weights = new List<playlist_dto>();
            DataTable dt = getallrecordsglobal();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                playlist_dto _playlist_dto = utilzsingleton.getInstance(_notificationmessageEventname).builddtogivendatatable(dt, i);
                lst_weights.Add(_playlist_dto);
            }
            return lst_weights;
        }
